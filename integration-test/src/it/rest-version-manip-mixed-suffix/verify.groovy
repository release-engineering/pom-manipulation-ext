...
assert dependency.version.text() == "1.0-redhat-temporary-1"

dependency = pom.dependencies.dependency.find { it.artifactId.text() == "errai-common" }
assert dependency != null
assert dependency.version.text() == "1.1-Final-redhat-temporary-1"

def passed = false
pom.properties.each {
    if ( it.text().contains ("3.1-redhat-temporary-1") )
    {
        passed = true
    }
}
